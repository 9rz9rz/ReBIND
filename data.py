...
class CIRR(Dataset):
    def __init__(self, preprocess, split, mode='relative', noise_ratio=0.0, args=None):
...
class FashionIQ(Dataset):
    def __init__(self, preprocess: callable, split: str, mode: str='relative', noise_ratio=0.0, specific_dress_type=None, args=None):
        ...
        for dress_type in self.dress_types:
            with open(self.path / 'captions' / f'cap.{dress_type}.{split}.json') as f:
                ...
                self.type_to_triplets[dress_type] = loaded_triplets 
        ...
        if self.split == 'train':
            ...
            for dress_type in self.dress_types:
                with open(self.path / 'image_splits' / f'split.{dress_type}.{split}.json') as f:
                    ...
                    self.image_type_names[dress_type] = loaded_names
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'relative':
            ...
            image_name = self.current_image_names[index]
            image_path = self.path / 'images' / f"{image_name}.png"
