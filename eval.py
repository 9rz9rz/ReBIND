...
def evaluate_features(model, dataset, text_preprocessor, similarity_option):
    acc_dict = {}
    ...
    if dataset_name == 'FashionIQ':
        ...
        for category in ['dress', 'shirt', 'toptee']:
            ...
            acc_dict[category] = compute_metric_features(model, dataset, text_preprocessor, similarity_option, category=category)
            acc_dict['acc'] += acc_dict[category]['acc']
        acc_dict['acc'] /= 3
        acc_dict['mean'] = acc_dict['acc']
        torch.cuda.empty_cache()
    else:
        acc_dict = compute_metric_features(model, dataset, text_preprocessor, similarity_option, category=dataset_name)
    logging.info(f"current metric acc: {acc_dict['acc']}")
    return acc_dict

def compute_metric_features(model, dataset, text_preprocessor, similarity_option, category):
    ...
    for idx in tqdm(range(0, len(dataset), query_bs), ncols=120, mininterval=30):
        ...
        distance:torch.Tensor = model.inference(F_r, F_images, captions, similarity_option)
